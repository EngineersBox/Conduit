...
public class EnumRefTransformer implements SchemaTransformer {
    ...
    private void affixEnum(final JsonNode parentNode,
        ...
        final JsonNode classNameNode = node.get(CLASS_FIELD_NAME);
        if (classNameNode == null || classNameNode.isNull() || classNameNode.isMissingNode()) {
            throw new IllegalStateException(String.format(
                    "%s$%s: expected \"%s\" field in definition",
                    TOKEN,
                    CLASS_FIELD_NAME,
                    CLASS_FIELD_NAME
            ));
    ...
    private Collection<? extends JsonNode> constructEnumNodes(final String className,
    ...
    @Override
    public ObjectNode transform(final ObjectNode schema) {
        JsonNode parentNode;
